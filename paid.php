...
$sql1="SELECT * FROM outstanding WHERE customer_id='".$customer_id."' and month='".($month3+1)."' and year='".$year."'";

  $result1=mysqli_query($con,$sql1);
  $arr1=mysqli_fetch_array($result1);

if (((($sum3*$unitprice)-((-$outstanding)+($sum2+$sum1+$sum+$transport+$other))))<0 && !($arr1['customer_id']==$customer_id) && !(($month3+1)==$arr1['month']) && !($year==$arr1['year'])){
	$sql="INSERT INTO outstanding(customer_id,month,year,price) VALUES ('".$customer_id."','".($month3+1)."','".$year."','".((($sum3*$unitprice)-((-$outstanding)+($sum2+$sum1+$sum+$transport+$other))))."')";
	$result=mysqli_query($con,$sql);
	if ($result){
		echo "<div align='center'>";
		echo "<h1 align='center'>හිඟ ශේෂයකි</h1>";
...
elseif (((($sum3*$unitprice)-((-$outstanding)+($sum2+$sum1+$sum+$transport+$other))))>0) {
		echo "<h1 align='center'>ධන ශේෂයකි. ශේෂය ඉදිරියට ගෙන යෑමට අවශයද?</h1>";
echo "<div align='center'>";
		echo "<a href='plusbalance.php'><button type='submit' class='btn btn-primary btn-lg' background-color='#3aa00b'><font style
='color: #040504'>ඔව්</font></button></a>";

echo "<br>";
echo "<br>";

echo "<a href='no.php'><button type='submit' class='btn btn-primary btn-lg' background-color='#3aa00b'><font style
='color: #040504'>නැත</font></button></a>";
echo "</div>";
}

elseif (((($sum3*$unitprice)+$outstanding)-($sum2+$sum1+$sum+$transport+$other))<0 && !($arr1['customer_id']=$customer_id) && !($arr1[
'month']==($month3+1)) && !($arr1['year']==$year)) {
	$sql="INSERT INTO outstanding(customer_id,month,year,price) VALUES ('".$customer_id."','".($month3+1)."','".$year."','".((($sum3*$unitprice)-((-$outstanding)+($sum2+$sum1+$sum+$transport+$other))))."')";
	$result=mysqli_query($con,$sql);
	if ($result){
		echo "<div align='center'>";
...
echo "</div>";
  ...
  echo "<h2>එක් වරක් ".$year." හි ".($month3+1)." මාසය සඳහා හිඟ ශේෂය ඇතුලත් කර ඇත</h2>";
  echo "<a href='pdfview1.php'><button type='submit' class='btn btn-primary btn-lg' background-color='#3aa00b'><font style
='color: #040504'>මුද්‍රණය</font></button></a>"; 
echo "<br>";
echo "<br>";
