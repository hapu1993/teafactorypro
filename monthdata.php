...
දළු කිලෝවකට උපරිම මිල අපෙන් මාවිටවත්ත තේ දළු එකතු කිරීමේ මධ්‍යස්ථානය </h3>
</div>
...
<select name="type" value="type">
  <option value="fertilizer">පොහොර</option>
  <option value="advance">අත්තිකාරම්</option>
  <option value="tea_leaves">තේ කොළ</option>
  <option value="dailytea">දෛනික තේ දළු කිරුම්</option>

</select>
<button type="submit" class="btn btn-primary btn-lg" background-color="#3aa00b" name="submit"><font style="color: #040504">Submit</font></button>
</form>
<?php
...
$date2="$year2/$month2/$day2";
	if ($type=='fertilizer'){
$sql="SELECT customer_id,price,date FROM fertilizer WHERE date>='".$date1."' and date<='".$date2."' ORDER BY date";
...
echo "<h2 align='center'>".$year1." හි ".$month1 ." මාසයේ ".$day1." සිට ".$year2." හි ".$month2." හි ".$day2." දක්වා තුල පොහොර විකුණුම්:</h2>";
...
echo "<table style='width:50%' align='center'>";
  ...
  while($arr=mysqli_fetch_array($result)){
  	$sql1="SELECT * FROM customer WHERE customer_id='".$arr['customer_id']."'";
  	$result1=mysqli_query($con,$sql1);
  	while($arr1=mysqli_fetch_array($result1)){
  echo "<tr>";
  	echo "<td>".$arr['date']."</td>";
    echo "<td>".$arr['customer_id']."</td>";
    echo "<td>".$arr1['customer_name']."</td>";
...
elseif ($type=='advance'){
$sql="SELECT customer_id,price,date FROM advance WHERE date>='".$date1."' and date<='".$date2."' ORDER BY date";
$result=mysqli_query($con,$sql);
...
echo "<h2 align='center'>".$year1." හි ".$month1 ." මාසයේ ".$day1." සිට ".$year2." හි ".$month2." හි ".$day2."
දක්වා අත්තිකාරම් මුදල්:</h2>";
echo "<table style='width:50%' align='center'>";
...
elseif ($type=='tea_leaves'){
$sql="SELECT customer_id,price,date FROM tea_leaves WHERE date>='".$date1."' and date='".$date2."' ORDER BY date";
$result=mysqli_query($con,$sql);
...
elseif ($type=='dailytea'){
$sql="SELECT customer_id,weight,date FROM dailytea WHERE date>='".$date1."' and date<='".$date2."' ORDER BY date";
$result=mysqli_query($con,$sql);
